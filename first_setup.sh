...
# ============================================================
# STEP 5.5: OpenCode CLI チェック（任意）
# ============================================================
log_step "STEP 5.5: OpenCode CLI チェック（任意）"

if command -v opencode &> /dev/null; then
    OPENCODE_VERSION=$(opencode --version 2>/dev/null || echo "unknown")
    log_success "OpenCode CLI がインストール済みです"
    if [ -n "$OPENCODE_VERSION" ] && [ "$OPENCODE_VERSION" != "unknown" ]; then
        log_info "バージョン: $OPENCODE_VERSION"
    fi
    RESULTS+=("OpenCode CLI: OK")
else
    log_warn "OpenCode CLI は未インストールです（任意）"
    echo ""
    echo "  OpenCode を使う場合のインストール:"
    echo "    npm install -g opencode-ai"
    echo ""

    OPENCODE_SHOULD_INSTALL=false
    OPENCODE_INSTALL_CHOICE="${INSTALL_OPENCODE:-}"

    case "${OPENCODE_INSTALL_CHOICE,,}" in
        1|true|yes|y)
            OPENCODE_SHOULD_INSTALL=true
            ;;
        0|false|no|n)
            OPENCODE_SHOULD_INSTALL=false
            ;;
        *)
            if [ ! -t 0 ]; then
                log_info "非対話環境のため OpenCode CLI の自動インストールをスキップします"
            else
                read -p "  OpenCode CLI をインストールしますか? [y/N]: " OPENCODE_REPLY
                OPENCODE_REPLY=${OPENCODE_REPLY:-N}
                if [[ "$OPENCODE_REPLY" =~ ^[Yy]$ ]]; then
                    OPENCODE_SHOULD_INSTALL=true
                fi
            fi
            ;;
    ...
    if [ "$OPENCODE_SHOULD_INSTALL" = true ]; then
        if command -v npm &> /dev/null; then
            log_info "OpenCode CLI をインストール中..."
            if npm install -g opencode-ai; then
                if command -v opencode &> /dev/null; then
                    OPENCODE_VERSION=$(opencode --version 2>/dev/null || echo "unknown")
                    log_success "OpenCode CLI インストール完了"
                    if [ -n "$OPENCODE_VERSION" ] && [ "$OPENCODE_VERSION" != "unknown" ]; then
                        log_info "バージョン: $OPENCODE_VERSION"
                    fi
                    RESULTS+=("OpenCode CLI: インストール完了")
                else
                    log_warn "npm install は完了しましたが opencode コマンドが見つかりません"
                    log_info "source ~/.bashrc または source ~/.zshrc の後で 'command -v opencode' を確認してください"
                    RESULTS+=("OpenCode CLI: PATH確認必要")
                fi
            else
                log_warn "OpenCode CLI のインストールに失敗しました（任意）"
                RESULTS+=("OpenCode CLI: インストール失敗 (任意)")
            fi
        else
            log_warn "npm が見つからないため OpenCode CLI をインストールできません"
            RESULTS+=("OpenCode CLI: スキップ (npm未インストール)")
        fi
    else
        RESULTS+=("OpenCode CLI: スキップ (任意)")
    fi
fi
...
#   agents:
#     ashigaru1:
#       type: claude   # claude / codex / copilot / kimi / opencode
#       model: sonnet
#     ashigaru2:
#       type: opencode
#       model: openrouter/openai/gpt-4o-mini
#       variant: high  # OpenCode の provider 固有オプション（任意）
EOF
...
echo ""
echo "  STEP B: OpenCode を使う場合の provider 設定（任意）"
echo "     opencode"
echo ""
echo "     1. /connect → OpenRouter など利用する provider を選択"
echo "     2. APIキーを登録"
echo "     3. /exit で退出"
echo ""
echo "     ※ headless運用では OPENROUTER_API_KEY など provider の環境変数を"
echo "       shutsujin_departure.sh を実行するシェルに読み込ませてください"
echo "     ※ config/settings.yaml の cli.agents.<agent>.type を opencode にすると"
echo "       OpenCode で起動できます"
echo ""
echo "  ────────────────────────────────────────────────────────────────"
echo ""
echo "  出陣（全エージェント起動）:"
